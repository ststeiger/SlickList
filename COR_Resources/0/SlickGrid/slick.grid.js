...
(function ($) {
  ...
  function SlickGrid(container, data, columns, options) {
    ...
    var defaults = {
      ...
      headerRowHeight: 25,
      showTopPanel: false,
      //showBorder: false, //REM: Kann womöglich durch eine Klasse selöst werden - ausstehend
      topPanelHeight: 25,
      formatterFactory: null,
      editorFactory: null,
      cellFlashingCssClass: "flashing",
      selectedCellCssClass: "slick-cell-selected",
      multiSelect: true,
	...
	function init(){
		...
		$headerRowScroller = $("<div class='slick-headerrow' style='position:relative;' />").appendTo($container);
		...
		//REM: Fusszeile
		$footerViewport = $('<div class="slick-viewport slick-footer-viewport">').css({'display': 'none', width: getCanvasWidth()}).appendTo($container);
		$footerRow = $('<div class="slick-row slick-footer-columns"></div>').css({position: 'relative'}).appendTo($footerViewport)
    ...
    function finishInitialization() {
      if (!initialized) {
        ...
        $headerScroller
			.bind("touchstart.slickgrid", handleHeaderTouchStart)
			.bind("touchend.slickgrid", handleHeaderTouchEnd)
        ...
        $canvas
			...
			.bind("touchstart.slickgrid", handleDblClick)
	...
	function updateCanvasWidth(forceColumnWidthsUpdate){
		...
		if(canvasWidth != oldCanvasWidth){
			...
			$topPanel.width(canvasWidth);
			$footerViewport.width(canvasWidth);
    ...
    function getMaxSupportedCssHeight() {
      ...
      var testUpTo = ($.browser && $.browser.mozilla) ? 6000000 : 1000000000;
    ...
    function updateColumnHeader(columnId, title, toolTip) {
		...
		//REM: Der Fehler stammt wohl aus der geänderten Reihenfolge für des Ein-/Ausblenden von Spalten.
		//REM: Müsste womöglich noch untersucht werden.
		//if($header){
		if(idx && $header){
			if(title !== undefined){
    ...
    function getHeaderRowColumn(columnId){
		var idx = getColumnIndex(columnId),
			$header = $headerRow.children().eq(idx);

		return $header && $header[0]
	};

	function createColumnHeaders(){
		for(var tL=$headers.get(0).querySelectorAll('.slick-header-column'), i=0, j=tL.length; i<j; i++){
			var columnDef = $(tL[i]).data("column");
			if(columnDef){
				trigger(self.onBeforeHeaderDestroy, {
					"headerNode": this,
					"column": columnDef
		...
		for(var i=0, j=columns.length; i<j; i++){
			...
			var header = $("<div class='slick-header-column' id='" + uid + m.id + "' />")
			...
			if(options.editable === true && typeof m.editor === 'function'){
				//REM: Obligatorische Spalten
				if(m.required){
					header.append("<span class='slick-required-indicator' />")
				}
				else{
					header.append("<span class='slick-edit-indicator' />")
				}
			};
	...
	//REM: Falls bereits ein stylesheet gesetzt wurde, gibt dies nichts mehr zurück.. scheint mir nicht sehr sinnvoll (?)
    function getColumnCssRules(idx) {
      if(!stylesheet){
	  //if(!stylesheet || !columnCssRulesL[idx]){
	...
	function applyColumnWidths(){
		for(var x=0, i=0, j=columns.length; i<j; i++){
    ...
    function setSortColumns(cols){
		...
		if(cols && cols.length){
			for(var i=0, j=cols.length; i<j; i++){
				if(cols[i].sortAsc == null) {
					cols[i].sortAsc = true
				};

				var columnIndex = getColumnIndex(cols[i].columnId);

				if(columnIndex != null){
					headerColumnEls.eq(columnIndex)
						.addClass("slick-header-column-sorted")
						.find(".slick-sort-indicator")
						.addClass(cols[i].sortAsc ? "slick-sort-indicator-asc" : "slick-sort-indicator-desc")
				}
			}
		}
    };
    ...
    function setColumns(columnDefinitions) {
      ...
      if (initialized) {
		...
		//REM: Habe diese zwei vor das Ereignis "createColumnHeaders()" genommen, da die neuen Styles da benötigt werden.
		//REM: Ob das so effektiv verhebt, weiss ich jedoch nicht :-)
		removeCssRules();
		createCssRules();
    ...
    function getDataItemValueForColumn(item, columnDef) {
      ...
      //REM: displayfield
      return columnDef.hasOwnProperty('displayfield') ? item[columnDef.displayfield] : item[columnDef.field];
    }
    ...
    function appendRowHtml(stringArray, row) {
      ...
      if (metadata && metadata.cssClasses) {
		...
		//REM: Meine eigenen Metadaten (dataview hat nur ein get, aber kein set?)
		var tMetadata = d && d._metadata;
		if(tMetadata && tMetadata.cssClasses){
			rowCss += " " + tMetadata.cssClasses
		};

      stringArray.push("<div class='" + rowCss + "' style='top:" + (options.rowHeight * row - offset) + "px'>");
      ...
      for (var i = 0, cols = columns.length; i < cols; i++) {
        ...
        if(d){
		  ...
		  //REM: Added options to the formatter for better slicklist support
          stringArray.push(getFormatter(row, m)(row, i, value, m, d, options))
        };
    ...
    function updateRow(row) {
      ...
      for (var columnIdx in cacheEntry.cellNodesByColumnIdx) {
        ...
        if (row === activeRow && columnIdx === activeCell && currentEditor) {
          ...
          node.innerHTML = getFormatter(row, m)(row, columnIdx, getDataItemValueForColumn(d, m), m, d, options);
        } else {
          node.innerHTML = "";
        }
      }
    ...
    function getViewportHeight(){
		//REM: $.css() beachtet "display: none" nicht.
		var tFooterHeight = $footerViewport.get(0).getBoundingClientRect().height;
    ...
    function handleHeaderTouchStart(e){
		//REM: Touchend hat keine Koordinaten.. braucht ja auch niemand :s
		touchStartObject = {
			Time: +new Date,
			x: ((e.touches && e.touches.length) ? e.touches[0] : e).clientX,
			y: ((e.touches && e.touches.length) ? e.touches[0] : e).clientY,
			px: ((e.touches && e.touches.length) ? e.touches[0] : e).pageX,
			py: ((e.touches && e.touches.length) ? e.touches[0] : e).pageY
		}
    };

    function handleHeaderTouchEnd(e){
		var tTimespanInSeconds = (+new Date - touchStartObject.Time) / 1000;
		if(touchStartObject && touchStartObject.Time && tTimespanInSeconds > 2){
			var $header = $(e.target).closest(".slick-header-column", ".slick-header-columns");
			var column = $header && $header.data("column");

			//REM: Das Touchend-Ereignis liefert keine Koordinaten, daher müssen diese manuell übergeben werden
			trigger(self.onHeaderContextMenu, {column: column, px: touchStartObject.px, py: touchStartObject.py}, e)
		};
	...
	//REM: Generiert die Fusszeile
	//REM: Fusszeile als Daten anzufügen gibt viele Probleme - zudem schneidet es sicht mit den SlickGrid-Eigenschaften "leaveSpaceForNewRows" und "enableAddRow"
	//REM: Fusszeile beschränkt sich, falls vorhanden, auf ausgewählte Zeilen
	function createFooter(){
        if($footerRow){
			$footerRow.empty();

			var tSelectedRows = selectionModel ? getSelectedRows() : null,
				tHasFooter = false;

			for(var i=0, j=columns.length; i<j; i++){
                var tColumn = columns[i],
					tValue = 0;

                if(typeof tColumn.footer === 'function'){
					tHasFooter = true;

                    for(var x=0, y=getDataLength(); x<y; x++){
						if(!tSelectedRows || !tSelectedRows.length || tSelectedRows.indexOf(x) !== -1){
							var tData = getDataItem(x);
							if(typeof tData[tColumn.field] === 'boolean'){
								tValue += +tData[tColumn.field]
							}
							else{
								tValue += (parseFloat(tData[tColumn.field], 10) || 0)
							}
						}
                    };

					tValue = tColumn.footer(tValue, y)
                }
				else{
					tValue = ''
				};

				var tCell = $footerRow.get(0).appendChild(document.createElement('div'));
				tCell.title = tCell.textContent = tValue;
				tCell.className = ('slick-cell slick-footer l@Index. r@Index. ' + tColumn.headerCssClass).replace(/@Index./g, i)
			};

			$footerViewport.toggle(tHasFooter)
        }
    };
