	...
	var _That = ns.Slicklist.Export = {
		Formatter: {
			...
			**************************************************************************************************/
	        ...
	        SVG: function(r, c, v, cd, dc, opt){ //REPLACE: async function(r, c, v, cd, dc, opt){
	            try{
        ...
        Init: function (slickgrid, event) {
            ...
            if (typeof srcElement === 'object' && (srcElement.innerHTML === 'PDF' || srcElement.innerHTML === 'CSV' || srcElement.innerHTML === 'XLSX') &&
                ...
                if (srcElement.innerHTML == 'PDF') _That.PDF._toPDF(slickgrid)
                else {
                    if (srcElement.innerHTML == 'CSV') setTimeout(function () { _That.Excel._toCSV(slickgrid) }, _That.timeOut)
                    else if (srcElement.innerHTML == 'XLSX') setTimeout(function () { _That.Excel._toXLSX(slickgrid) }, _That.timeOut)
                }              
            }
        },
        ...
        PDF: {
            ...
            generatePDF: function (Job) {
                ...
                try {
                    var suffix = '';
                    if (Job['Parameters']['File_Count'] > 1) suffix = ' [' + (Job['Parameters']['File_Index'] + 1 + ']')
        ...
        Excel: {
            ...
            getSheetColumnWidth: function (Job) {
                ...
                if (Job.Columns) {
                    ...
                    for (var i = 0; i < Job.Columns.length; i++) {
                        ...
                        if (tWidth != undefined || tMinWidth === tWidth) sheetWidth -= tWidth; //fix widths
                        else sumColumnWidth += tMinWidth; //variable widths         
            ...
            getSheetRowHeight: function (Job) {
                ...
                for (var i = 1; i < Job.Data.length; i++) { wsrows.push({ hpx: height }); }
        ...
        Data: {
            ...
            _ListToJob: function  (param) { //REPLACE: async function(param){
                ...
                if (tColumns && slickgrid.dataView) {
                        ...
                        if (dc['__group'] === true) {
                            ...
                            for (var i = 0; i < tColumns.length; i++) {
                                ...
                                if (typeof col['footer'] === 'function') {
                                    value = dc[field];
                                    if(!isNaN(parseFloat(value)) && isFinite(value)) tFooter[col.id] += parseFloat(value);
                                }
                            }
                        }
                        tDataArray.push(row)
                    ...
                    if (type != 'CSV' && Object.keys(tFooter).length > 0) {
                        ...
                        for (var i = 0; i < tColumns.length; i++) {
                            ...
                            if (type === 'PDF') row.push({ text: value, style: 'footer' })
                            else row.push(value)   
                        }
