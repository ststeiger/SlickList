...
(function make_xlsx(XLSX) {
    ...
    if (typeof module !== "undefined" && typeof require !== 'undefined') {
        if (typeof cptable === 'undefined') global.cptable = require('./cpexcel.js');
    }
    function reset_cp() {
        set_cp(1200);
    }
        ...
        for (var i = 0; i < images.length; i++) {
            ...
            if (pos.type === 'twoCellAnchor') {
                ...
                var parent = pos.parent || {},
                    ...
                    pWidth = col.wpx || 0, pHeight = row.hpx || 0;

                //fix height
                pWidth = Math.floor(pWidth / 100 * 135);
                pHeight = Math.floor(pHeight / 100 * 130);
                             
                var pcx = Math.floor(pWidth * 914400 / 96)
                var pcy = Math.floor(pHeight * 914400 / 96)

                var cx = Math.floor(width * 914400 / 96)
                var cy = Math.floor(height * 914400 / 96)

                 
                var ratio = [pcx / cx, pcy / cy ];ratio = Math.min(ratio[0], ratio[1]);
                cx =  Math.floor(cx*ratio);
                cy =  Math.floor(cy*ratio);
                
                var colOff = Math.floor((pcx - cx) / 2);
                var rowOff = 5000 + Math.floor((pcy - cy) / 2);
                ...
                var oneCell =                   
                    ...
                    '<xdr:pic>' +
                        '<xdr:nvPicPr>' +
                            '<xdr:cNvPr id="' + (i + 1) + '" name="' + image.name + '"/>' +
                            '<xdr:cNvPicPr>' +
                                '<a:picLocks noChangeAspect="1"/>' +
                            '</xdr:cNvPicPr>' +
                        '</xdr:nvPicPr>' +
                        '<xdr:blipFill>' +
                            '<a:blip xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" r:embed="rId' + (i + 1) + '" cstate="print"/>' +
                            '<a:stretch>' +
                                '<a:fillRect/>' +
                            '</a:stretch>' +
                        '</xdr:blipFill>' +
...
// 2.3.2
    ...
    function format_cell(cell, v) {
        ...
        if (cell.w !== undefined) return cell.w;
        //FIX CSV Export
        if (v === undefined || v === null) return safe_format_cell(cell, cell.v);
